...
// ConnGorm 连接数据库
func ConnGorm(c config.Config) *gorm.DB {
	// 使用自定义日志
	conn, err := gorm.Open(
		mysql.Open(c.DB.DataSource),
		&gorm.Config{Logger: NewGormLogger(c.Mode)},
	)
	if err != nil {
	...
	connPoolInit(c, conn)
	conn.Logger.Info(context.Background(), "连接数据库成功", err)
	return conn
}

// 连接池初始化
func connPoolInit(c config.Config, conn *gorm.DB) {
